...
def uploads():
    if request.method == 'POST':
        ...
        model = genai.GenerativeModel('gemini-1.5-flash')
        response = model.generate_content(["Based on this question answer in simple words , If it is a page with full od text then read and summarie the text ,if it is a book cover page then  give full  summary of the book Highlight the best things about the book and read out atleast 20  lines inside that book etc ,if it is a currency tell the currency note , if it is a scenary describe the scenary if it is question text then answer for the question in short so your role is to give answer to question or summary to text or describing a scenary or identifying a currency etc act according to the situation ", img], stream=True)
        response.resolve()
        rply=response.text
...
@app.route('/ansupload', methods=['POST'])
def queupload():
    if request.method == 'POST':
        # Get the image data from the request
        data = request.json
